...
class CyberRangeCreation():
    ...
    def __init__(self, argv):
        ...
        try:
            opts, args = getopt.getopt(
                argv, "hdv", [
                    "help", "destroy-on-error", "verbose"])
        except getopt.GetoptError as err:
            print(
                "* ERROR: cyris: Command-line argument error: %s" %
                (str(err)))
            self.usage()
            quit(-1)
        ...
        for opt, arg in opts:
            if opt in ("-h", "--help"):
                ...
                print(
                    "* INFO: cyris: In case of error, will try to destroy cyber range.")
        ...
        print("#########################################################################")
        print(
            "%s: Cyber Range Instantiation System" %
            (self.get_version_string()))
        print("#########################################################################")

        # Get global parameters from CONFIG file.
        print("* INFO: cyris: Parse the configuration file.")
        ABS_PATH, CR_DIR, GW_MODE, GW_ACCOUNT, GW_MGMT_ADDR, GW_INSIDE_ADDR, USER_EMAIL = parse_config.parse_config(
            args[1])
        ...
        self.time_measure_file = "{0}logs/cr_creation_time.txt".format(
            ABS_PATH)

        # Absolute path of file which records ip addresses of current running
        # base images
        self.cur_running_ipaddr_file = "{0}settings/running_ipaddr.txt".format(
            ABS_PATH)
        ...
        # a list of config files created in individual hosts during cloning
        # part
        # for setting up fwrule on each vms
        self.setup_fwrule_file = ""
        # for setting up default gateways on each vms
        self.setup_dfgw_file = ""
        # for creating bridges of each cyber range instance
        self.create_bridges_file = ""
        # for individual hosts to check ping their entry points after vms are
        # created
        self.entry_points_file = ""
        self.clone_file = ""                            # for cloning process
        # for creating vms from base images
        self.create_vms_file = ""
        # for creating ssh tunnels for entry points
        self.create_tunnels_file = ""
        # for creating random accounts and passwds on entry points
        self.create_entry_accounts_file = ""
        # for installing wordpress on webservers (only needed in level 2)
        self.install_wordpress_file = ""
        # for destructing cyber range on individual hosts
        self.destruction_file = ""
        # for containing a list of hosts for parallel-ssh
        self.pssh_file = ""
        # for containing a list of hosts for parallel-scp
        self.pscp_file = ""
        # for copying post-cloned programs on other hosts
        self.prepare_prg_afcln_file = ""
        # for executing programs on individual guest after being cloned
        self.install_prg_afcln_file = ""
        # for returning the creation process result: success or failure
        self.creation_status_file = ""
        # for recording log of the creation process
        self.creation_log_file = ""

        if GW_MODE:
            if GW_ACCOUNT is None or GW_MGMT_ADDR is None or GW_INSIDE_ADDR is None:
                print(
                    "* ERROR: cyris: If GW_MODE is enabled in the config file, then GW_ACCOUNT, GW_MGMT_ADDR and")
    ...
    # FIXME: Should call error handling function so that execution is
    # terminated
    ...
    def add_basevm_ipaddr(self, last_bit):
        try:
            ...
            # Therefore, it needs only to create the file, and write the
            # last_bit down.
            ...
            # Else if the file exists.
                ...
                with open(self.cur_running_ipaddr_file, "r+") as f:
                    ...
                    # Original code; should work now as every written number is
                    # followed by EOL
                    ...
                    # If the file isn't empty.
                        ...
                        i = 0
                        # Iterate the current addr list to check if the last
                        # bit has been existed.
                        while (i < len(cur_lb_list)):
                            if DEBUG:
                                print(
                                    "* DEBUG: cyris: Current element in ipaddr list:", int(cur_lb_list[i]))
    ...
    def parse_description(self, filename):
        try:
            ...
            print(
                "* ERROR: cyris: Issue with the cyber range description file: ",
                exc)
            return

        # for each playbook in the training description
        for element in doc:
            if "host_settings" in list(element.keys()):
            ...
            if "guest_settings" in list(element.keys()):
                for i, g in enumerate(element["guest_settings"]):
                    # If user specifies ip_addr for the guest.
                    if "ip_addr" in list(g.keys()):
                        ip_addr = g["ip_addr"]
                    # Else, CyRIS generates ip_addr for the guest using its own
                    # rules as below.
                    else:
                        # Assign last bit of IP addr for the guest, which is
                        # the sum of 100 and the guest's i.
                        last_bit = self.add_basevm_ipaddr(100 + i)
                        # Generate IP addr for the guest, which is
                        # 192.168.122.{last_bit}.
                        ip_addr = "192.168.122.{0}".format(last_bit)
                    # If user specifies tasks for the guest.
                    if "tasks" in list(g.keys()):
                        tasks = g["tasks"]
                    else:
                        tasks = []
                    # If basevm_os_type defined
                    if "basevm_os_type" in list(g.keys()):
        ...
        if "clone_settings" in list(element.keys()):
            ...
            for host in element["clone_settings"][0]["hosts"]:
                ...
                for host_id in host_id_list:
                    ...
                    for i in range(1, instance_num + 1):
                        ...
                        # recreate a clone_guest_list and clone_subnw_list
                        # when creating a new instance.
                        clone_subnw_list = []
                        for subnetwork in host["topology"][0]["networks"]:
                            name = subnetwork["name"]
                            members = subnetwork["members"]
                            if "gateway" in list(subnetwork.keys()):
                                gateway = subnetwork["gateway"]
                            else:
                                gateway = ""
                            clone_subnetwork = CloneSubnetwork(
                        ...
                        for guest in host["guests"]:
                            ...
                            number = guest["number"]
                            fw_rules = []
                            if Storyboard.FORWARDING_RULES in list(
                                    guest.keys()):
                                has_fw_setup = True
                                ...
                                has_fw_setup = False
                            if "entry_point" in list(guest.keys()):
                                is_entry_point = True
                            else:
                            ...
                            for k in range(1, number + 1):
                                ...
                                clone_guest = CloneGuest(
                                    guest_id, k, has_fw_setup, fw_rules, is_entry_point, os_type)
                                clone_guest_list.append(clone_guest)
                        instance = CloneInstance(
                            i, clone_guest_list, clone_subnw_list)
                        instance_list.append(instance)

                    # Find host with same id in the hosts list and use it to initialize the CloneHost object
                    # We use a new object 'actual_host' because the object
                    # 'host' above may contain a list of host ids
                    actual_host = self.get_host_object(host_id)
                    clone_host = CloneHost(actual_host, instance_list)
                    clone_host_list.append(clone_host)
            self.clone_setting = CloneSetting(
                range_id, nw_type, clone_host_list)

        # Set basevm_name for guests
        for guest in self.guests:
            basevm_name = "{0}_cr{1}_base".format(
                guest.getGuestId(), self.clone_setting.getRangeId())
    ...
    #########################################################################
    # Set names for directory and config files related to the being created
    # cyber range
    def set_config_file_name(self):
        ...
        self.directory = "{0}{1}/".format(CR_DIR,
                                          self.clone_setting.getRangeId())
        ...
        self.create_bridges_file = "{0}create_bridges.sh".format(
            self.directory)
        ...
        self.create_tunnels_file = "{0}create_tunnels.sh".format(
            self.directory)
        self.create_entry_accounts_file = "{0}create_entry_accounts.sh".format(
            self.directory)
        self.install_wordpress_file = "{0}install_wordpress.sh".format(
            self.directory)
        self.destruction_file = "{0}destruct_cyberrange.sh".format(
            self.directory)
        self.pssh_file = "{0}pssh_hosts.txt".format(self.directory)
        self.pscp_file = "{0}pscp_hosts.txt".format(self.directory)
        self.prepare_prg_afcln_file = "{0}prepare_prg_afcln.sh".format(
            self.directory)
        self.install_prg_afcln_file = "{0}install_prg_afcln.sh".format(
            self.directory)
        self.creation_status_file = "{0}cr_creation_status".format(
            self.directory)
    ...
    def get_instantiation_commands(self, guest):
        ...
        # Post_execute_program_list is a list programs that need to perform on
        # the cloned guests after the cloning phase.
        command_list = []
        ...
        host_mgmt_addr, host_virbr_addr, host_account = self.get_host(
            guest.getBasevmHost())
        command = ""
        # Parse commands from tasks if it is not empty.
        if len(guest.getTasks()) != 0:
            for task in guest.getTasks():
                if "add_account" in list(task.keys()):
                    for account in task["add_account"]:
                        new_account = account["account"]
                        new_passwd = account["passwd"]
                        if Storyboard.FULL_NAME in list(account.keys()):
                            full_name = account[Storyboard.FULL_NAME]
                        else:
                            full_name = ""

                        command = ManageUsers(
                            guest_addr, ABS_PATH).add_account(
                            new_account, new_passwd, full_name, guest_os_type)
                        command_list.append(command)

                if "modify_account" in list(task.keys()):
                    for account in task["modify_account"]:
                        ...
                        if "new_passwd" in account:
                            ...
                            new_passwd = "null"
                        command = ManageUsers(
                            guest_addr, ABS_PATH).modify_account(
                            old_account, new_account, new_passwd, guest_os_type)
                        command_list.append(command)
                ...
                if "emulate_traffic_capture_file" in list(task.keys()):
                    for fi in task["emulate_traffic_capture_file"]:
                        ...
                        generateTraffic = GenerateTrafficCaptureFiles(
                            host_virbr_addr,
                            guest_addr,
                            guest_passwd,
                            attack_type,
                        ...
                        if attack_type == "ssh_attack":
                            ...
                            command = generateTraffic.ssh_attack(
                                host_account, attack_source, 50)
                            command_list.append(command)
                        if attack_type == "dos_attack":
                            attack_source = fi["attack_source"]
                            if "attack_port" in list(fi.keys()):
                                attack_port = fi["attack_port"]
                            else:
                                attack_port = "80"
                            command = generateTraffic.dos_attack(
                                attack_source, attack_port)
                            command_list.append(command)
                        if attack_type == "ddos_attack":
                            command = generateTraffic.ddos_attack()
                            command_list.append(command)

                if "emulate_malware" in list(task.keys()):
                    for malware in task["emulate_malware"]:
                        ...
                        command = EmulateMalware(
                            guest_addr, name, mode, crspd_option, ABS_PATH).command()
                        command_list.append(command)

                if "copy_content" in list(task.keys()):
                    for content in task["copy_content"]:
                        src = content["src"]
                        dst = content["dst"]
                        command = CopyContent(
                            src, dst, guest_addr, guest_passwd, ABS_PATH, guest_os_type).command()
                        command_list.append(command)
                ...
                if "execute_program" in list(task.keys()):
                    for program in task["execute_program"]:
                        program_name = program["program"]
                        interpreter = program["interpreter"]
                        args = "none"
                        if "args" in list(program.keys()):
                            #                            args = program["args"]
                        ...
                        # to the command_list. Otherwise, the program will be
                        # added to the post_execute_program_list
                        if "execute_time" not in list(program.keys(
                        )) or program["execute_time"] == "before_clone":
                ...
                if "firewall_rules" in list(task.keys()):
                    for ruleset in task["firewall_rules"]:
                        ruleset_file = ruleset["rule"]
                        command = ModifyRuleset(
                            guest_addr, guest_passwd, ruleset_file, ABS_PATH).command()
                        command_list.append(command)
    ...
    def shut_down_baseimg(self):
        shutdown_command = ""
        for guest in self.guests:
            shutdown_command += "virsh --quiet shutdown {0} > /dev/null;".format(
                guest.getBasevmName())
            shutdown_command += "virsh --quiet undefine {0} > /dev/null;".format(
                guest.getBasevmName())
            # Only run the command below if the running ipaddress file exists
            if os.path.isfile(self.cur_running_ipaddr_file):
                shutdown_command += "sed -i '/{0}/d' {1};".format(
                    guest.getAddrLastBit(), self.cur_running_ipaddr_file)
        return shutdown_command

    #########################################################################
    # Summary commands lists for all base images into a dictionary
    # <guest_id>:<command_list>
    def cyberrange_instantiation_commands(self):
        ...
        for guest in self.guests:
            dict_guest_cmd_bfcln[guest.getGuestId()], dict_guest_prg_afcln[guest.getGuestId(
            )] = self.get_instantiation_commands(guest)
    ...
    def os_system(self, filename, command):
        # Make sure the command is executed even if no filename is provided for
        # the log file
        if filename:
        ...
        if exit_status != 0:
            print(
                "* ERROR: cyris: Issue when executing command (exit status = %d):" %
                (exit_status))
            print("  %s" % (command))
            print(
    ...
    def clone_vm_commands(self, dict_guest_prg_afcln):
        ...
        # This function is to check and generate new ports for current cyber
        # range instances
        # Contains a set of used ports for previous cyber range instances
        used_port_list = []
        process_list = []
        p = subprocess.Popen(
            r"ps -aux | grep 'ssh -f -L \| {0}@localhost'".format(
                MSTNODE_ACCOUNT),
            shell=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT)
        ...
        for line in process_list:
            for element in line.split(None, nfields):
                if "0.0.0.0".encode("ascii") in element:
                    used_port = element.decode("ascii").split(":")[1]
                    used_port_list.append(used_port)

        if DEBUG2:
            print((
                "* DEBUG: cyris: clone_vm_commands: INITIAL: used_port_list={}".format(used_port_list)))

        # Generate fresh ports for entry points
        # Generate a list with ports between a minimum and maximum value (both
        # inclusive)
        MIN_PORT_NO = 60000
        MAX_PORT_NO = 65000
        fresh_port_list = list(range(MIN_PORT_NO, MAX_PORT_NO + 1))
        if DEBUG2:
            print((
                "* DEBUG: cyris: clone_vm_commands: INITIAL: fresh_port_list={}".format(fresh_port_list)))
        ...
        if DEBUG2:
            print((
                "* DEBUG: cyris: clone_vm_commands: FILTERED: fresh_port_list={}".format(fresh_port_list)))
        ...
        if DEBUG2:
            print((
                "* DEBUG: cyris: clone_vm_commands: SHUFFLED: fresh_port_list={}".format(fresh_port_list)))
        ...
        self.range_details_filename = "{0}{1}{2}.yml".format(
            self.directory, RANGE_DETAILS_FILE, self.clone_setting.getRangeId())
    ...
    def check_ssh_connectivity(self, check_command, if_addr):
        ...
        if DEBUG:
            ...
            print(
                "* DEBUG: cyris:       Timeout time for SSH check:",
                timeout_time)
        ...
        while datetime.now() < timeout_time:
            ...
            # Check whether CHECK_SSH_CONNECTIVITY_INDICATOR string can be
            # found
            if CHECK_SSH_CONNECTIVITY_INDICATOR.encode("ascii") not in stderr:
                if DEBUG:
                    print(
                        "* DEBUG: cyris:       Check SSH connectivity to {0} => FAILURE".format(if_addr))
            else:
                if DEBUG:
                    print(
                        "* DEBUG: cyris:       Check SSH connectivity to {0} => SUCCESS".format(if_addr))
                return
    ...
    def check_ssh_connectivity_to_basevms(self):
        if DEBUG:
            print("* DEBUG: cyris: Checking SSH connectivity for base VMs...")
        ...
        for guest in self.guests:
            ...
            if DEBUG:
                print(
                    "* DEBUG: cyris: - Checking base VM for guest '{0}' ({1})...".format(
                        guest.getGuestId(),
                        guest.getBasevmAddr()))
            ...
            OPTIONS = "-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PasswordAuthentication=no -o ConnectTimeout={0}".format(
                CHECK_SSH_TIMEOUT_ONCE)
            check_command = "ssh {0} {1} ls".format(OPTIONS, if_addr)
            if DEBUG:
                print(
                    "* DEBUG: cyris:       Command: {0}".format(check_command))

            # Call function that does the actual check
            self.check_ssh_connectivity(check_command, if_addr)

    #########################################################################
    # Check whether all cloned machines in the cyber range can be accessed via
    # SSH
    def check_ssh_connectivity_to_cr(self):
        if DEBUG:
            print("* DEBUG: cyris: Checking SSH connectivity for cyber range...")
        ...
        for host in self.clone_setting.getCloneHostList():
            ...
            if DEBUG:
                print(
                    "* DEBUG: cyris: - Checking instances on host '{0}' ({1})...".format(
                        host.getHostId(), host.getMgmtAddr()))
            ...
            for instance in host.getInstanceList():
                ...
                if DEBUG:
                    print(
                        "* DEBUG: cyris:   + Checking instance #{0}...".format(instance.getIndex()))
                ...
                for clone in instance.getCloneGuestList():
                    ...
                    if DEBUG:
                        print(
                            "* DEBUG: cyris:     - Checking cloned guest '{0}'...".format(clone.getGuestId()))

                    # Check whether clone has NICs
                    if clone.getNicAddrDict():

                        # Get first interface address for this guest from
                        # address list
                        ...
                        OPTIONS_CR = OPTIONS_MGMT + \
                            " -o ConnectTimeout={0}".format(
                                CHECK_SSH_TIMEOUT_ONCE)
                        check_command = "ssh {} {} 'ssh {} {} ls'".format(
                            OPTIONS_MGMT, host.getMgmtAddr(), OPTIONS_CR, if_addr)
                        if DEBUG:
                            print(
                                "* DEBUG: cyris:       Command: {0}".format(check_command))
                        ...
                        print(
                            "* WARNING: cyris: No NIC defined for cloned guest '{0}'.".format(clone.getGuestId()))
    ...
    def send_email(self, username):
        ...
        if "{ID}" in contents:
            contents = contents.replace(
                "{ID}", str(self.clone_setting.getRangeId()))
        if "{num_cr_instances}" in contents:
            contents = contents.replace(
                "{num_cr_instances}\n",
                "- Total number of cyber range instances: {0}".format(
                    self.clone_setting.getTotalInstanceNum()))
        information = ""
        instance_index = 1
                ...
                for host in self.hosts:
                    if instance.getEntryPoint().getHostId() == host.getHostId():
                        ...
                        information += "\n\n- Cyber range instance #{0}:\n  Login: ssh {1}@{2} -p {3}\n  Password: {4}".format(
                            instance_index, entry_point.getAccount(), host_name, entry_point.getPort(), entry_point.getPasswd())
                        instance_index += 1
                        break
        ...
        self.range_notification_filename = "{0}{1}{2}.txt".format(
            self.directory, RANGE_NOTIFICATION_FILE, self.clone_setting.getRangeId())
        ...
        if USER_EMAIL is not None:
            ...
            sendemail_command = "sendemail -f '" + EMAIL_SENDER + "' -t {0} -u 'Training Session #{1} Is Ready' -s " + \
                EMAIL_SERVER + \
                " -o tls=yes -o message-file={2}{3}{1}.txt -a {2}{4}{1}.yml -xu " + \
                EMAIL_ACCOUNT + " -xp " + EMAIL_PASSWD

            # Copy the email to the gateway to send to user if it's the gateway
            # mode.
            if GW_MODE:
                command = "scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {0}{1}{2}.txt {3}@{4}:/tmp; scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {0}{5}{2}.yml {3}@{4}:/tmp;".format(
                    self.directory, RANGE_NOTIFICATION_FILE, self.clone_setting.getRangeId(), GW_ACCOUNT, GW_INSIDE_ADDR, RANGE_DETAILS_FILE)
                ...
                command += "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {0}@{1} \"{2}\"".format(
                    GW_ACCOUNT, GW_INSIDE_ADDR, sendemail_command)

            # Send the email directly from the master node if it's the
            # un-gateway mode.
            else:
    ...
    def copy_base_images(self):
        ...
        for guest in self.guests:
            ...
            command += "cp {0}.xml {1}{2}; cp {0} {1}{3};".format(
                original_files, self.directory, xml_config_file, guest.getGuestId())
            ...
            command += "sed -i 's#<name>.*#<name>{0}</name>#' {1}{2};".format(
                guest.getBasevmName(), self.directory, xml_config_file)
            # Replace original location of base image in config file with new
            # one.
            command += "sed -i \"s#<source file=.*#<source file='{0}{1}'/>#\" {0}{2};".format(
                self.directory, guest.getGuestId(), xml_config_file)
            # Replace original mac addr with new one as guest' IP addr
            guest_last_bit = guest.getBasevmAddr().rsplit(".")[-1]
            command += "sed -i \"s#<mac.*#<mac address='52:54:00:f8:30:{0}'/>#\" {1}{2};".format(
                hex(int(guest_last_bit))[2:], self.directory, xml_config_file)
            # Set the basevm_config_file of guest to be the new one.
            guest.setBasevmConfigFile(
                "{0}{1}".format(
                    self.directory,
                    xml_config_file))
    ...
    def main(self):
        ...
        ssh_host_file = "{0}settings/{1}pssh_host.txt".format(
            ABS_PATH, self.clone_setting.getRangeId())
        scp_host_file = "{0}settings/{1}pscp_host.txt".format(
            ABS_PATH, self.clone_setting.getRangeId())

        # Get the IP address of the current host
        crt_host_ipaddr = socket.gethostbyname(socket.gethostname())
        if DEBUG:
            print(("Current host IP address: " + crt_host_ipaddr))

        # Open files with truncation, so that any potential incorrect value is
        # overwritten
        with open(ssh_host_file, "w+") as pssh, open(scp_host_file, "w+") as pscp:
            for host in self.hosts:
                ...
                if host.getMgmtAddr() != crt_host_ipaddr:
                    logging.debug(
                        "Target '{0}' is different from current host ({1}) => do pscp".format(
                            host.getMgmtAddr(), crt_host_ipaddr))
                    pscp.write(
                        "{0}:{1} {2}\n".format(
                            host.getMgmtAddr(),
                            22,
                    ...
                    logging.debug(
                        "Target '{0}' is same with current host ({1}) => skip pscp".format(
                            host.getMgmtAddr(), crt_host_ipaddr))
        ...
        parallel_ssh_command = "parallel-ssh -O StrictHostKeyChecking=no -O UserKnownHostsFile=/dev/null -i -h {0} -t {1} -p {2} -x '-tt'".format(
            ssh_host_file, PSSH_TIMEOUT, PSSH_CONCURRENCY)

        ######## Create directory on individual hosts #########
        command_to_run = "mkdir -p {0}; mkdir {0}images;".format(
            self.directory)
        command = "{0} \"{1}\"".format(parallel_ssh_command, command_to_run)

        # Run command
        # TODO: Should use os_system function instead of calling os.system
        # directly?!
        if DEBUG:
            print(command)
        ...
        if exit_status != 0:
            print(
                "* ERROR: cyris: Issue when creating the directory '%s'." %
                (self.directory))
            print(
        ...
        for guest in self.guests:
            command = BaseImageLaunch(
                guest.getBasevmConfigFile(),
                guest.getBasevmName(),
                ABS_PATH).command()
            self.os_system(self.creation_log_file, command)
        ...
        for guest in self.guests:
            ...
            command = SSHKeygenHostname(
                guest.getBasevmAddr(),
                guest.getRootPasswd(),
                guest.getGuestId(),
                MSTNODE_ACCOUNT,
                ABS_PATH,
                guest.basevm_os_type).command()
            ...
            # add default gw
            # FIXME: Should use the virbr_addr value instead of the fixed IP
            # below
            if guest.basevm_os_type in ('windows.7'):
                ...
                command = "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(
                    guest.getBasevmAddr(), add_gw_str)
                add_gw_str = "route add 0.0.0.0 mask 0.0.0.0 192.168.122.1"
                command += "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(
                    guest.getBasevmAddr(), add_gw_str)
            elif guest.basevm_os_type in ('centos.7'):
                command = "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} route add default gw 192.168.122.1".format(
                    guest.getBasevmAddr())
            if DEBUG:
                print(command)
            self.os_system(self.creation_log_file, command)
        ...
        for guest in self.guests:
            ...
            for command in commands:
                ...
                with open(self.creation_log_file, "a") as myfile:
                    if isinstance(command.getCommand(), list):
        ...
        # execute self.clone_vm_commands() function (which links to clas
        # to create config files and get necessary inform
        ...
        # Check whether shutdown completed for all base VMs before distributing
        # images
        if DEBUG:
            print(
                "* DEBUG: cyris: Checking whether shutdown completed for all base VMs...")
        for guest in self.guests:
            if DEBUG:
                print(
                    "* DEBUG: cyris: - Checking guest '{0}' base VM...".format(guest.getGuestId()))

            spin = True
            while spin:
                output = subprocess.check_output(["virsh", "list", "--all"])
                print(output)
                spin = guest.getBasevmName().encode("ascii") in output
                if DEBUG:
                    print(
                        "* DEBUG: cyris:   Base VM '{0}' is still running => SLEEP".format(guest.getBasevmName()))
                time.sleep(2)
            if DEBUG:
                print(
                    "* DEBUG: cyris:   Base VM '{0}' was undefined => CONTINUE".format(guest.getBasevmName()))
        ...
        if TIME_MEASURE == True:
            start_scp = time.time()
        # Check if the pscp file is empty. If yes, then there's no need for
        # calling parallel-scp
        if os.stat(scp_host_file).st_size != 0:
            command = ""
            parallel_scp_command = "parallel-scp -O StrictHostKeyChecking=no -O UserKnownHostsFile=/dev/null -h {0} -p {1}".format(
                scp_host_file, PSCP_CONCURRENCY)
            for guest in self.guests:
                # NOTE: The name of the file in which a guest base VM image is stored is given by getGuestId()
                #       However, the name of the VM itself, as registered in KVM, and the associated XML file
                # are given by getBasevmName() -- See the 'copy_base_images'
                # function
                command += "{0} {1}{2}* {1} &\n".format(
            ...
            # Idea: add some bash code to check exit status via "$?" and return
            # -1 on error
            self.os_system(self.creation_log_file, command)
        ...
        clone_command = "chmod +x {0}; bash -x {0}; ".format(self.create_bridges_file)
        print(clone_command)
        os.system(clone_command)
        # execute clone.sh script
        clone_command = "chmod +x {0}; bash -x {0}; ".format(self.clone_file)
        print(clone_command)
        os.system(clone_command)
        # create tunnels
        clone_command = "chmod +x {0}; bash -x {0}; ".format(self.create_tunnels_file)
        print(clone_command)
        os.system(clone_command)

        # TDG: only one host is used so removing these unnecessary bits
        # if DEBUG:
        #     print(clone_command)
        # self.os_system(
        #     self.creation_log_file,
        #     "{0} \"{1}\"".format(
        #         parallel_ssh_command,
        #         clone_command))
        ...
        cea_command = "chmod +x {0}; {0};".format(
            self.create_entry_accounts_file)
        if DEBUG:
            print(cea_command)
        self.os_system(
        ...
        # Install command.
        install_command = "chmod +x {0}; {0};".format(
            self.install_prg_afcln_file)
        if DEBUG:
            ...
            for instance in host.getInstanceList():
                for clone_guest in instance.getCloneGuestList():
                    if clone_guest.getOsType().find("windows") != -1:
                        ...
                        command = "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} tsdiscon 1".format(
                            clone_guest.getNicAddrDict()["eth0"])
                        self.os_system(self.creation_log_file, command)
        ...
        with open("{0}whole-controlled-destruction.sh".format(self.directory), "w+") as f:
            f.write(
                "echo \"* INFO: cyris: Destroy cyber range with id {0} and clean up.\"\n".format(self.range_id))
            f.write("command_1=\"{0}\"\n".format(self.destruction_file))
            f.write(
                "{0} \"${{command_1}}\" > /dev/null\n".format(parallel_ssh_command))
            f.write("command_2=\"rm -rf {0}\"\n".format(self.directory))
            f.write(
                "{0} \"${{command_2}}\" > /dev/null\n".format(parallel_ssh_command))
            f.write("rm {0}; rm {1}\n".format(ssh_host_file, scp_host_file))
        self.os_system(self.creation_log_file,
                       "chmod +x {0}whole-controlled-destruction.sh".format(self.directory))
        ...
        if TIME_MEASURE == True:
            ...
            print(
                "  Instantiation details: %s" %
                (self.range_details_filename))
            print(
                "  Login notification: %s" %
                (self.range_notification_filename))
        print("-------------------------------------------------------------------------")
    ...
    def handle_error(self, is_fatal=True):
        ...
        if not is_fatal and os.path.isfile(self.cur_running_ipaddr_file):
            ...
            for guest in self.guests:
                erase_command += "sed -i '/{0}/d' {1};".format(
                    guest.getAddrLastBit(), self.cur_running_ipaddr_file)
        ...
        if not is_fatal and DESTROY_ON_ERROR:
            print(
                "* INFO: cyris: Execution error => try to destroy cyber range and clean up.")
            ...
            if os.path.exists(
                    "{0}whole-controlled-destruction.sh".format(self.directory)):
                self.os_system(self.creation_log_file,
                               "{0}whole-controlled-destruction.sh".format(self.directory))
            else:
                if self.directory:
                    self.os_system(self.creation_log_file,
                                   "rm -rf {0}".format(self.directory))
