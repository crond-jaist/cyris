...
class VMClone(object):
    def __init__(self, gw_mode, gw_account, gw_addr, host_list, guest_list, clone_setting, log_file, setup_fwrule_file, setup_dfgw_file, create_bridges_file, entry_points_file,
                 clone_file, create_vms_file, create_tunnels_file, create_entry_accounts_file, install_prg_afcln_file, install_wordpress_file, destruction_file, directory, abspath):
        ...
        # management address of the whole cyber range
        self.mgmt_addr = "{0}.0.0.0/8".format(self.clone_setting.getRangeId())
    ...
    #########################################################################
    # Generate ipaddr & netmask for the initif.conf file, and copy it to the
    # base images
                    ...
                    for clone_guest in instance.getCloneGuestList():
                        if guest.getGuestId() == clone_guest.getGuestId():
                            ...
                            for i in range(0, nic_num):
                                initif_mac_addr_5 = "%x" % int(clone_guest.getNicAddrDict()[
                                                               "eth" + str(i)].split(".")[2])
                                initif_mac_addr_6 = "%x" % int(clone_guest.getNicAddrDict()[
                                                               "eth" + str(i)].split(".")[3])
                                content += "eth{0} {1} {2}:{3}\n".format(
                                    i, self.mgmt_addr, initif_mac_addr_5.zfill(2), initif_mac_addr_6.zfill(2))
                            with open(filename, "w") as f:
                                f.write(content)
                            if guest.getBasevmOSType() == 'windows.7':
                                command = r"scp {0}initif.conf root@{1}:'C:\CyberRange\initif'".format(
                                    self.directory, guest.getBasevmAddr())
                            else:
                                command = "scp {0}initif.conf root@{1}:/bin/cyberrange/initif".format(
                                    self.directory, guest.getBasevmAddr())
                            os.system(
                                command +
                                ">> {0} 2>&1".format(
    ...
    def create_bridges(self):
        ...
        for clone_host in self.clone_setting.getCloneHostList():
            ...
            # print "* NOTE: cyris: The warning below can be safely ignored
            # (caused by use of paramiko library in 'clone_environment.py')."
            ...
            with sftp_client.open(self.create_bridges_file, "a+") as myfile:
                myfile.write(SHEBANG)
                for instance in clone_host.getInstanceList():
                    for bridge in instance.getBridgeList():
                        # Write bridge config in /etc/network/interface by
                        # calling 01_write_bridge_config.sh
                        myfile.write(
                            "sudo -S {0}{3}/vm_clone/create_bridges/01_write_bridge_config.sh {1} {2}\n".format(
                                self.abspath, bridge.getId(), bridge.getAddr(), INSTANTIATION_DIR))
                        # TDG: https://askubuntu.com/a/967271, after 16.04 this command needs to be done beforehand
                        myfile.write("sudo /etc/init.d/networking restart\n".format(bridge.getId()))
                        # Call ifup bridge
                        myfile.write(
                            "sudo -S ifup br{0} &\n".format(bridge.getId()))
            ...
            with sftp_client.open(self.clone_file, "a+") as f:
                ...
                for instance in clone_host.getInstanceList():
                    ...
                    for clone_guest in instance.getCloneGuestList():
                        ...
                        # vm_id =
                        # <guest_id>_<range_id>_<instance_index>_<guest_index>,

                        # The KVM domain name is now defined in entities.py,
                        # writeConfig function
                        if clone_guest.kvm_domain:
                            if DEBUG:
                                print(
                                    "* DEBUG: KVM domain name already defined => use it.")
                            vm_id = clone_guest.kvm_domain  # Save KVM domain name
                        else:
                            print(
                                "* WARNING: KVM domain name not yet defined => generate now.")
                            vm_id = "{0}_cr{1}_{2}_{3}".format(
                                clone_guest.getGuestId(),
                                self.clone_setting.getRangeId(),
                                instance.getIndex(),
                                clone_guest.getIndex())

                        # Generate bridge_id_list and addr_list for the vm
                        # addr_list consists of ip addresses of every nic in the vm
                        # bridge_id_list consists of bridge_id of every nic in
                        # the vm
                        bridge_id_list = []
                        addr_list = []
                        for key, value in list(
                                clone_guest.getNicAddrDict().items()):
                            addr_list.append(value)
                            bits_vmaddr = value.split(".")
                            for bridge in instance.getBridgeList():
                                bits_bridge = bridge.getAddr().split(".")
                                if bits_bridge[0] == bits_vmaddr[0] and bits_bridge[
                                        1] == bits_vmaddr[1] and bits_bridge[2] == bits_vmaddr[2]:
                        ...
                        f.write(
                            "{0}{6}/vm_clone/create_vms.sh 1 {1} {2} {0} {3} {4} {5};\n".format(
                                self.abspath,
                                vm_id,
                                clone_guest.getGuestId(),
                                self.directory,
                    ...
                    for guest in instance.getCloneGuestList():
                        if len(guest.getFwRuleList()) != 0:
                            ...
                            command += "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(
                                list(guest.getNicAddrDict().values())[0], rule_str)
                ...
                for instance in clone_host.getInstanceList():
                    ...
                    for guest in instance.getCloneGuestList():
                        if len(guest.getNicGwDict()) != 0:
                            for nic, gw in list(guest.getNicGwDict().items()):
                                if guest.getOsType() == "windows.7":
                                    add_gw_str = "route delete 0.0.0.0 mask 0.0.0.0"
                                    command += "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(
                                        list(guest.getNicAddrDict().values())[0], add_gw_str)
                                    add_gw_str = "route add 0.0.0.0 mask 0.0.0.0 {0}".format(
                                        gw)
                                    command += "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(
                                        list(guest.getNicAddrDict().values())[0], add_gw_str)
                                else:
                                    add_gw_str = "route add default gw {0} {1}".format(
                                        gw, nic)
                                    command += "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(
                                        list(guest.getNicAddrDict().values())[0], add_gw_str)
        ...
        for clone_host in self.clone_setting.getCloneHostList():
            ...
            with sftp_client.open(self.create_tunnels_file, "a+") as tunnel_file:
                ...
                for instance in clone_host.getInstanceList():
                    ...
                    if self.gw_mode:
                        # Create tunnel on the crond-gw machine with tunnel
                        # name is "ct{range_id}{port}".
                        command = "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {0}@{1} -f \"bash -c 'exec -a ct{2}_{3} ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -f -L 0.0.0.0:{3}:{4}:{3} {0}@localhost -N'\";\n".format(
                            self.gw_account, self.gw_addr, self.clone_setting.getRangeId(), instance.getEntryPoint().getPort(), mgmt_addr)
                        ...
                        command += "bash -c 'exec -a ct{0}_{1} ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -f -L 0.0.0.0:{1}:{2}:22 {3}@localhost -N'\n".format(
                            self.clone_setting.getRangeId(), instance.getEntryPoint().getPort(), instance.getEntryPoint().getAddr(), account)
            ...
            with sftp_client.open(self.create_entry_accounts_file, "a+") as entry_file:
                # For each entry element in the dictionary of {key="entry_point
                # addr", value="port"}.
                for instance in clone_host.getInstanceList():
                    ...
                    command = ManageUsers(
                        ...
                        self.abspath).add_account(
                        instance.getEntryPoint().getAccount(),
                        instance.getEntryPoint().getPasswd(),
                        FULL_NAME,
                        os_type).getCommand()
                    entry_file.write("{0};\n".format(command))
                ...
                for instance in clone_host.getInstanceList():
                    ...
                    if self.gw_mode:
                        command = "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {0}@{1} \"pkill -f ct{2}_{3}\";\n".format(
                            self.gw_account, self.gw_addr, self.clone_setting.getRangeId(), instance.getEntryPoint().getPort())
                        command += "pkill -f ct{0}_{1};\n".format(
                            self.clone_setting.getRangeId(), instance.getEntryPoint().getPort())
                    else:
                        command = "pkill -f ct{0}_{1};\n".format(
                            self.clone_setting.getRangeId(), instance.getEntryPoint().getPort())
            ...
            with sftp_client.open(self.install_prg_afcln_file, "a+") as install_file:
                for instance in clone_host.getInstanceList():
                    for guest in instance.getCloneGuestList():
                        ...
                        if guest.getGuestId() in list(dict_guest_prg_afcln.keys()):
                            ...
                            for program in program_list:
                                install_file.write(
                                    "{0};\n".format(
                                        program.command_post_clone(guest_addr).getCommand()))

    # This function is for executing scripts after the cloning step. User copies the file to the
    # base image in advance using the "copy" task, then uses this function to
    ...
    def create_destruction_file(self):
        for host in self.host_list:
            ...
            with sftp_client.open(self.destruction_file, "a+") as f:
                # down the bridges
                f.write(
                    "sudo python3 {0}{1}/downbridges.py /etc/network/interfaces {2};\n".format(
                        self.abspath,
                        CLEANUP_DIR,
                        self.clone_setting.getRangeId()))
                # delete vms
                ...
                f.write("{0}{3}/vm_clone/vm_destroy_xml.sh {1} {2};\n".format(self.abspath,
                                                                              self.clone_setting.getRangeId(), self.directory, INSTANTIATION_DIR))
